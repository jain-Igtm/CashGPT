...
function githubHeaders(token) {
  return {
    ...
    "User-Agent": "cashgpt-live-runner",
...
function buildAgentInput({ agent, agents, transcript, seedContext, round }) {
  ...
  return [
    `You are ${agent.name}, one participant in CashGPT's live room.`,
    agent.instructions,
...
async function main() {
  ...
  const issueNumber = parsePositiveInteger(process.env.ROOM_ISSUE, 1, 1_000_000);
